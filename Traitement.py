...
@author: ANTIER Paul, BOESSINGER Nicolas, PEGON Etienne, SAVARY Anatole, SCHULLER Baptiste
"""
...
temp_casserole = 55; # On définit la température max d'analyse avant laquelle on considère qu'un pixel appartient à une casserole
bornemask = (temp_casserole + 183) / 0.007 # On convertit cette température en son équivalent en intensité de pixel
vide = np.zeros((120,160),dtype = bool)
w=0

figure() # Définition de la fonction d'affichage
def draw_fig():
    plt.imshow(IMG)

# Traitement
while(1): 
    if (w>9): # On lit nos images dans un buffer de 10 images
        w=0 
    w+=1
    try:
